"""Offline checks for the composability manifest (``src/noaa_weather/catalog.yaml``).
...
.ffl sources, declared as `workflow`/`facet` respectively), and there are no
duplicate entries. No runner, DB, or network needed.
"""
...
def test_manifest_loads(manifest):
    ...
    assert manifest.get("package") == "noaa-weather"
...
def test_event_facets_in_ffl_are_catalogued(ffl_text):
    """Every `event facet` declared in the FFL carries Effect+Cost and is indexed.

    Guards against drift: a new event facet without composability metadata, or
    missing from the manifest, fails here.
    """
    # Leaf names of event facets declared in the FFL.
    event_leaves = set(re.findall(r"\bevent facet\s+(\w+)\b", ffl_text))
    catalogued = {f["qualified_name"].rsplit(".", 1)[-1] for f in catalog.facets()}
    missing = event_leaves - catalogued
    assert not missing, f"event facets missing from catalog.yaml: {sorted(missing)}"
